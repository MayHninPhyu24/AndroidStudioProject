...
class ThemeScreen extends StatelessWidget {
  ...
  Widget buildRadioListTile(ThemeMode themeVal, String txt, IconData? icon,BuildContext ctx){
    return RadioListTile(
      ...
      value: themeVal,
      groupValue: Provider.of<ThemeProvider>(ctx,listen: false).tm,
            ...
            Expanded(
                child: ListView(
                  children: [
                    ...
                    buildRadioListTile(ThemeMode.system,"${lan.getTexts('System_default_theme')}", null, context),
                    buildRadioListTile(ThemeMode.light, "${lan.getTexts('light_theme')}", Icons.wb_sunny_outlined, context),
                    buildRadioListTile(ThemeMode.dark, "${lan.getTexts('dark_theme')}", Icons.nights_stay_outlined, context),
                    buildListTile(context, "primary",lan),
                    buildListTile(context, "accent" ,lan),
  ...
  ListTile buildListTile(BuildContext context, txt,lan) {
    ...
    return ListTile(
      title: Text("${ (txt == 'primary')? lan.getTexts('primary'): lan.getTexts('accent')}",
        style: Theme.of(context).textTheme.headline6,
      ),
                ...
                content: SingleChildScrollView(
                  child: ColorPicker(
                    ...
                    onColorChanged: (newColor) =>
                      Provider.of<ThemeProvider>(context, listen: false).onChanged(newColor,
                          txt == "primary" ? 1: 2),
